...
public class Player implements Bot {
    ...
    private static enum Strategy {FOLDER, WATCHER, AGGRESSIVE}
    ...
    public Player() {
        ...
        this.strategy = new Strategy[] {Strategy.WATCHER, Strategy.WATCHER, Strategy.WATCHER};
...
//        List<String> myCards = roundState.hands.get(active);  // your six cards at the start of the round
    ...
    public List<Action> getActions(GameState gameState, RoundState roundState, int active) {
        ...
        for (int i = 0; i < State.NUM_BOARDS; i++) {
            ...
            if (legalBoardActions.contains(ActionType.ASSIGN_ACTION_TYPE)) { // default assignment of hands to boards
                myActions.add(new Action(ActionType.ASSIGN_ACTION_TYPE, List.of(optimalAllocation.get(2*i), 
                        optimalAllocation.get(2*i + 1))));
            }
     ...
     * is where the sum of the win probability of each board pair is maximized.
    ...
    private List<String> getOptimalAllocation(List<String> cards){
        ...
        for (List<String> a : allocations) {
            double probSum = getWinProbability(a.get(0), a.get(1)) + 
                    getWinProbability(a.get(2), a.get(3)) +
                    getWinProbability(a.get(4), a.get(5));
            if (probSum > maxProb) {
                optimalAllocation = a;
                maxProb = probSum;
            }
